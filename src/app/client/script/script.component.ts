...
export class ScriptComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.renderer.appendChild(
      script,
      this.renderer.createText(`${document.getElementById("_script_" + this.uuid)?.innerHTML.replace(/\@\[/g, "{").replace(/\]\@/g, "}")}`)
    );

    const scriptContainer = this.el.nativeElement.querySelector(
      `#_script_${this.uuid}`
    );
    if (scriptContainer) {
